...
export const validatePhone = (contactCode, contactNumber) => {
  if (!contactNumber) {
    return { isValid: false, error: 'Required' };
  }
  
  // Remove spaces, hyphens, parentheses, etc. to get only digits
  const cleanNumber = contactNumber.trim().replace(/\D/g, '');
  
  if (cleanNumber.length === 0) {
    return { isValid: false, error: 'Required' };
  }

  // Exact dialing code configuration rules
  const rules = {
    '+91': { lengths: [10], regex: /^[6-9]\d{9}$/, errorMsg: 'Must be 10 digits starting with 6-9' },
    '+61': { lengths: [9], regex: /^4\d{8}$/, errorMsg: 'Must be 9 digits starting with 4 (e.g. 412345678)' },
    '+1': { lengths: [10], regex: /^[2-9]\d{9}$/, errorMsg: 'Must be 10 digits (e.g. 2015550123)' },
    '+44': { lengths: [10], regex: /^7\d{9}$/, errorMsg: 'Must be 10 digits starting with 7' },
    '+971': { lengths: [9], regex: /^5\d{8}$/, errorMsg: 'Must be 9 digits starting with 5' },
    '+64': { lengths: [8, 9, 10], regex: /^\d{8,10}$/, errorMsg: 'Must be 8-10 digits' },
    '+92': { lengths: [10], regex: /^3\d{9}$/, errorMsg: 'Must be 10 digits starting with 3' },
    '+880': { lengths: [10], regex: /^1\d{9}$/, errorMsg: 'Must be 10 digits starting with 1' },
    '+65': { lengths: [8], regex: /^[89]\d{7}$/, errorMsg: 'Must be 8 digits starting with 8 or 9' },
    '+60': { lengths: [9, 10], regex: /^1\d{8,9}$/, errorMsg: 'Must be 9-10 digits starting with 1' },
    '+33': { lengths: [9], regex: /^[67]\d{8}$/, errorMsg: 'Must be 9 digits starting with 6 or 7' },
    '+49': { lengths: [10, 11], regex: /^1[5-7]\d{8,9}$/, errorMsg: 'Must be 10-11 digits starting with 15/16/17' },
    '+39': { lengths: [9, 10], regex: /^3\d{8,9}$/, errorMsg: 'Must be 9-10 digits starting with 3' },
    '+34': { lengths: [9], regex: /^[67]\d{8}$/, errorMsg: 'Must be 9 digits starting with 6 or 7' },
    '+353': { lengths: [9], regex: /^8\d{8}$/, errorMsg: 'Must be 9 digits starting with 8' },
    '+86': { lengths: [11], regex: /^1[3-9]\d{9}$/, errorMsg: 'Must be 11 digits starting with 1' },
    '+966': { lengths: [9], regex: /^5\d{8}$/, errorMsg: 'Must be 9 digits starting with 5' },
    '+974': { lengths: [8], regex: /^[3567]\d{7}$/, errorMsg: 'Must be 8 digits' },
    '+968': { lengths: [8], regex: /^9\d{7}$/, errorMsg: 'Must be 8 digits starting with 9' },
    '+965': { lengths: [8], regex: /^[569]\d{7}$/, errorMsg: 'Must be 8 digits' },
    '+973': { lengths: [8], regex: /^3\d{7}$/, errorMsg: 'Must be 8 digits starting with 3' },
    '+27': { lengths: [9], regex: /^[6-8]\d{8}$/, errorMsg: 'Must be 9 digits' }
  };

  const rule = rules[contactCode];
  if (rule) {
    if (!rule.lengths.includes(cleanNumber.length) || (rule.regex && !rule.regex.test(cleanNumber))) {
      return { isValid: false, error: rule.errorMsg };
    }
  } else {
    // Generic fallback for any other country code
    if (cleanNumber.length < 5 || cleanNumber.length > 15) {
      return { isValid: false, error: 'Must be 5-15 digits' };
    }
  }
